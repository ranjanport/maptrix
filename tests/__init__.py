...
class TestLibMethods(unittest.TestCase):
    ...
    test_cases = [
        ...
        {"name": "Near Quito", "lon": -78.45, "lat": 0.0, "expected": (-935944956, 0)},
    ]
    ...
    def test_to_nds_coordinates(self):
        ...
        for case in self.test_cases:
            ...
            assert isinstance(
                result, tuple
            ), f"{case['name']}: Expected tuple, got {type(result)}"

            # Test that tuple has exactly 2 elements
            assert (
                len(result) == 2
            ), f"{case['name']}: Expected tuple of length 2, got {len(result)}"

            # Test that both elements are integers
            assert isinstance(
                result[0], int
            ), f"{case['name']}: x-coordinate should be int, got {type(result[0])}"
            assert isinstance(
                result[1], int
            ), f"{case['name']}: y-coordinate should be int, got {type(result[1])}"
    ...
    def test_wgs2nds_with_coord2pointxy(self):
        ...
        for case in self.test_cases:
            ...
            result = CoordinateMath().WGS2NDS(
                Coord2PointXY(long=case["lon"], lat=case["lat"])
            )
    ...
    def test_nds2wgs_with_coord2pointxy(self):
        ...
        for case in test_cases:
            ...
            tolerance = 0.0001
            assert (
                abs(result.long - case["expected"].long) < tolerance
            ), f"{case['name']}: Expected long={case['expected'].long}, got {result.long}"
            assert (
                abs(result.lat - case["expected"].lat) < tolerance
    ...
    def test_decode_morton_code(self):
        ...
        test_cases = [
            {"mcode": 519168122092038698, "wgs": (77.16796875, 28.55417418912454)},
            {"mcode": 579221254078012839, "wgs": (2.2945, 48.858222)},
            {"mcode": 5973384896724652798, "wgs": (-74.044444, 40.689167)},
            {"mcode": 8983442095026671932, "wgs": (-43.157444, -22.948658)},
            {"mcode": 4354955230616876489, "wgs": (151.214189, -33.857529)},
            {"mcode": 585611620934393888, "wgs": (0.0, 51.503)},
            {"mcode": 5782627506097029136, "wgs": (-78.45, 0.0)},
        ]
    ...
    def test_get_tile_id_from_wgs84(self):
        ...
        nds_coord = Coord2PointXY(long=77.158708, lat=28.556217).to_nds_coordinate
        mcode = MortonCode(
